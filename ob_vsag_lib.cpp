...
create_index(VectorIndexPtr& index_handler,
    ...
    if (is_support) {
        ...
        nlohmann::json index_parameters{
            {"dtype", dtype}, {"metric_type", metric}, {"dim", dim}, {"glass", hnsw_parameters}};
        if (auto index =
                vsag::Factory::CreateIndex("glass", index_parameters.dump(), vsag_allocator);
            index.has_value()) {
            hnsw = index.value();
...
knn_search(VectorIndexPtr& index_handler,
    ...
    nlohmann::json search_parameters{{"glass", {{"ef_search", ef_search}}}};
    HnswIndexHandler* hnsw = static_cast<HnswIndexHandler*>(index_handler);
...
fdeserialize(VectorIndexPtr& index_handler, std::istream& in_stream) {
    ...
    nlohmann::json hnsw_parameters{{"max_degree", max_degree},
                                   ...
                                   {"use_static", use_static}};
    nlohmann::json index_parameters{
        {"dtype", "float32"}, {"metric_type", "l2"}, {"dim", dim}, {"glass", hnsw_parameters}};
    vsag::logger::debug("   Deserilize hnsw index , index parameter:{}, allocator addr:{}",
                        index_parameters.dump(),
                        (void*)hnsw->get_allocator());
    if (auto index =
            vsag::Factory::CreateIndex("glass", index_parameters.dump(), hnsw->get_allocator());
        index.has_value()) {
