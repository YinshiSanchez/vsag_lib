...
struct MaxHeap {
    ...
    void
    sift_down(int i, int u, dist_t dist) {
        pool[0] = {u, dist};
        for (; 2 * i + 1 < size;) {
            int j = i;
            int l = 2 * i + 1, r = 2 * i + 2;
...
inline int
insert_into_pool(Neighbor* addr, int K, Neighbor nn) {
