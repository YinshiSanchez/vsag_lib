...
#include "quantization/glass_fp32_quant.hpp"
...
struct SQ8Quantizer {
    ...
    explicit SQ8Quantizer(int dim)
        : d(dim),
          ...
          code_size(d_align),
          mx(d_align, -HUGE_VALF),
          mi(d_align, HUGE_VALF),
          dif(d_align) {
    }
    ...
    void
    encode(const float* from, char* to) const {
        for (int j = 0; j < d; ++j) {
    ...
    void
    reorder(const Pool& pool, const float* /**q*/, int* dst, int k) const {
        for (int i = 0; i < k; ++i) {
            dst[i] = pool.id(i);
        }
    }
    ...
    struct Computer {
        ...
        Computer(const SQ8Quantizer& quant, const float* query)
            : quant(quant),
              q((float*)alloc64B(quant.d_align * 4)),
              mi(quant.mi.data()),
              dif(quant.dif.data()) {
        ...
        operator()(int u) const {
            return dist_func(q, (data_type*)quant.get_data(u), quant.d_align, mi, dif);
        }
        void
    ...
    serialize(std::ostream& writer) const {
        ...
        writer.write((char*)codes, code_size);
